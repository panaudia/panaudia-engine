...
// meters-native (design decision 9 — positions arrive pre-converted, no
// size anywhere below here), one distance/attenuation computation per pair,
// direction optionally rotated into the listener's head frame. M5 extends
// this same pass to emit both ears' directions.
func pairGeometry(listenerPosMeters common.Position,
	peerPosMeters common.Position,
	headFrame *common.RotationMatrix33,
	peerGainFactor float32,
	peerAttenuationExponent float64) (norm common.Position, distanceMeters float64, nodeGain float32) {
	...
	return norm, distanceMeters, peerGainFactor * distanceGain(distanceMeters, peerAttenuationExponent)
}
...
// first REVERB_CHANNELS of the dry weights take dryGain.
func reverbSplit(distanceMeters float64) (dryGain, wetGain float32) {
...
func GetWeights(weights []float32,
	...
	norm, _, nodeGain := pairGeometry(listenerPosMeters, peerPosMeters, headFrame,
		peerGainFactor, peerAttenuationExponent)
...
func GetWeightsForReverb(dryWeights []float32,
	...
	dryGain, wetGain := reverbSplit(distanceMeters)
