...
func TestChannelRouting(t *testing.T) {
	...
	m.SetChannel("stage", ChannelParams{Muted: true, Gain: 1, Attenuation: 2})
	settle(m, 5)
...
func TestExplicitEmptyChannelsMeansNone(t *testing.T) {
	...
	// Explicit empty source-channel list: deliberately audible to nobody
	// (the base profile's "explicit empty array means deliberately none").
